...
func solution(_ S : String) -> String {
    ...
    let noSpacesNoDashes = noSpaces.replacingOccurrences(of: "-", with: "")
    
    if noSpacesNoDashes.count <= 3 {
        return noSpacesNoDashes
    }
    
    // lopping through char-by-char append dashes
    var result = ""
    
    var count = -2
    for c in noSpacesNoDashes {
        result.append(c)
        if count % 3 == 0 {
            result.append("-")
        }
