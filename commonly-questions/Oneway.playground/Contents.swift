...
func oneAway(_ first: String, _ second: String) -> Bool {
    ...
    // If the strings differ by > 1 lenght, they are not one replacement away
    // If the strings are the same lenght,  they are one replacement away
    // If the strings differ by one,        they may be one edit or insert away
    // If the stirngs differ by one less,   they may be one edit or insert away
...
// Do this by comparing character and pointers.
//
// For example:
//      pale
//      i
...
//      j
//
// As soon as you detect that one of characters dosen't match,
// compare the indexes. If they are the same, that's OK. That means
// this is yout first check dosen't match. Increment the other pointer.
// If the characters don't march again, and the pointers don't match,
